...
import { albumList, mediaItemsSearch } from "./google-photos";
...
program
  ...
  .action(async (cmd) => {
    try {
      ...
      console.log(
        response.albums.map((a: any) => ({ id: a.id, title: a.title }))
      );
    } catch (error) {
      console.error(error);
    }
...
async function downloadAlbum(tokens: TokenSet, config: Config) {
  let items: any[] = [];
  const pageSize = 50;
  let response = await mediaItemsSearch(
    tokens.access_token!,
    config.albumId,
    pageSize
  );
  ...
  while (response.nextPageToken) {
    response = await mediaItemsSearch(
      tokens.access_token!,
      config.albumId,
      pageSize,
...
//         const fileAlreadyExists = existsSync(
//           `${config.workingFolder}/album/${filename}`
//         );
...
async function getTokens(config: Config): Promise<TokenSet> {
  ...
  console.debug(
    `expires_at: ${tokens.expires_at} - date = ${new Date().getTime() / 1000}`
  );
  if (tokens.expired()) {
